    ...
    <mt-tab-container v-model="selected">
      <mt-tab-container-item id="1">
        <div class="list list-box" @click="toCash">
          ...
          <p>共收到{{list1 && list1.length ? list1.length : 0}}个礼物，合计￥{{countInfo ? countInfo.balance : 0}}元</p>
        </div>
        <mygiftlist v-if="list1 && list1.length>0" :list="list1" type="1"></mygiftlist>
        <div v-else class="empty">没有记录</div>
      </mt-tab-container-item>
      <mt-tab-container-item id="2">
        <mygiftlist v-if="list2 && list2.length>0" :list="list2" type="2"></mygiftlist>
        <div v-else class="empty">没有记录</div>
      </mt-tab-container-item>
    </mt-tab-container>
  </div>
